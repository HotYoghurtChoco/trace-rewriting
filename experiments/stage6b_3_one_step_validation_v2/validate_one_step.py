...
This executable implements the original v1 pre-registration together with the
diagnostic-informed v2 protocol amendment.  It does not train a student, run
OPRO, or reinterpret the Stage 6B-2 result.
"""
...
METHOD_VERSION = "stage6b_3_one_step_validation_v2"
EXPECTED_BRANCH = "research/gradient-feedback"
EXPECTED_PREREG_COMMIT = "7e2f8221d3e51e28959899b28aa37b3de896868e"
EXPECTED_AMENDMENT_COMMIT = "7a504251b7b1616017f46d74d530ec830cb5d7f8"
...
def rank_top50(values: Sequence[float]) -> set[int]:
    if len(values) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("Top-50 audit requires exactly 100 values")
    ranked = sorted(
...
def main() -> None:
    ...
    print(f"protocol_amendment_commit: {EXPECTED_AMENDMENT_COMMIT}")
    ...
    require_sha(
        amendment_path,
        EXPECTED_SHA256["protocol_amendment"],
        "protocol_amendment",
    )
    ...
    def validate_candidate_token_metadata(candidate_index: int) -> None:
    ...
    print("\n===== V2 NO-UPDATE VECTOR REPRODUCIBILITY PREFLIGHT =====")
    ...
    preflight_trainable_sha_before = named_parameter_sha256(trainable)
    if preflight_trainable_sha_before != theta0_sha:
        raise RuntimeError("Preflight did not begin from theta0")
    frozen_versions_before_preflight = {
    ...
    for candidate_index in range(EXPECTED_CANDIDATE_COUNT):
        reset_to_theta0()
        if trainable_mismatch_count() != 0:
            raise RuntimeError("Preflight candidate did not begin from theta0")
        validate_candidate_token_metadata(candidate_index)

        score_row = score_rows[candidate_index]
        frozen_dot = finite_number(
            float(score_row["gradient_dot"]),
            "frozen_gradient_dot",
        )
        frozen_candidate_norm = finite_number(
            float(score_row["candidate_gradient_norm"]),
            "frozen_candidate_gradient_norm",
        )
        denominator = finite_number(
            frozen_candidate_norm * frozen_reference_norm,
            "frozen_gradient_norm_product",
        )
        if denominator <= 0.0:
            raise RuntimeError(
                f"Candidate {candidate_index}: frozen norm product is not positive"
            )
        ...
        absolute_error = finite_number(
            abs(recomputed_dot - frozen_dot),
            "gradient_dot_absolute_error",
        )
        normalized_discrepancy = finite_number(
            absolute_error / denominator,
            "gradient_dot_normalized_discrepancy",
        )
        old_v1_tolerance = max(1e-6, 1e-4 * abs(frozen_dot))
        ...
        preflight_rows.append(
            {
                ...
                "gradient_dot_absolute_error": absolute_error,
                "frozen_candidate_gradient_norm": frozen_candidate_norm,
                "frozen_mean_reference_gradient_norm": frozen_reference_norm,
                "frozen_gradient_norm_product": denominator,
                "normalized_dot_discrepancy": normalized_discrepancy,
                "v2_normalized_discrepancy_limit": (
                    NORMALIZED_DOT_DISCREPANCY_MAX
                ),
                "within_v2_normalized_discrepancy_limit": (
                    normalized_discrepancy <= NORMALIZED_DOT_DISCREPANCY_MAX
                ),
                "old_v1_gradient_dot_tolerance": old_v1_tolerance,
                "within_old_v1_gradient_dot_tolerance": (
                    absolute_error <= old_v1_tolerance
                ),
                "recomputed_candidate_loss": candidate_loss,
                "recomputed_candidate_gradient_norm": candidate_norm,
            }
        )
        frozen_dots.append(frozen_dot)
        recomputed_dots.append(recomputed_dot)
        absolute_errors.append(absolute_error)
        normalized_discrepancies.append(normalized_discrepancy)

        reset_to_theta0()
        if trainable_mismatch_count() != 0:
            raise RuntimeError("Preflight candidate changed theta0")
        del candidate_gradient
        torch.cuda.empty_cache()
        if (candidate_index + 1) % 10 == 0:
            print(f"v2_preflight_candidate_progress: {candidate_index + 1}/100")

    preflight_spearman = spearman_correlation(frozen_dots, recomputed_dots)
    preflight_pearson = pearson_correlation(frozen_dots, recomputed_dots)
    frozen_top50 = rank_top50(frozen_dots)
    recomputed_top50 = rank_top50(recomputed_dots)
    sign_agreement_count = sum(
        int(np.sign(frozen) == np.sign(recomputed))
        for frozen, recomputed in zip(
            frozen_dots,
            recomputed_dots,
            strict=True,
        )
    )
    old_v1_tolerance_pass_count = sum(
        int(row["within_old_v1_gradient_dot_tolerance"])
        for row in preflight_rows
    )
    max_normalized_discrepancy = max(normalized_discrepancies)
    preflight_trainable_sha_after = named_parameter_sha256(trainable)
    preflight_frozen_sha_after = named_parameter_sha256(frozen)
    frozen_versions_after_preflight = {
        name: parameter._version for name, parameter in frozen
    }
    parameter_integrity_pass = (
        preflight_trainable_sha_after == preflight_trainable_sha_before
        and preflight_frozen_sha_after == frozen_parameter_sha_before
        and frozen_versions_after_preflight == frozen_versions_before_preflight
        and trainable_mismatch_count() == 0
    )
    preflight_pass = (
        preflight_spearman >= REPRODUCIBILITY_SPEARMAN_MIN
        and max_normalized_discrepancy <= NORMALIZED_DOT_DISCREPANCY_MAX
        and parameter_integrity_pass
    )

    preflight_summary = {
        "status": "PASS" if preflight_pass else "FAIL",
        "method_version": METHOD_VERSION,
        "scope": "all_100_candidates_no_update_vector_reproducibility",
        "candidate_count": len(preflight_rows),
        "reference_count": EXPECTED_REFERENCE_COUNT,
        "positive_eta_update_performed": False,
        "scientific_endpoint_evaluated": False,
        "optimizer_created": False,
        "frozen_stage6b_1_dot_remains_inferential_predictor": True,
        "pass_fail_criteria": {
        ...
        "spearman_frozen_vs_recomputed": preflight_spearman,
        "pearson_frozen_vs_recomputed": preflight_pearson,
        "sign_agreement_count": sign_agreement_count,
        "sign_agreement_total": EXPECTED_CANDIDATE_COUNT,
        "top50_overlap_count": len(frozen_top50 & recomputed_top50),
        "top50_changed_membership_count": (
            len(frozen_top50 ^ recomputed_top50) // 2
        ),
        "absolute_error": quantile_summary(absolute_errors),
        "normalized_error_by_frozen_norm_product": quantile_summary(
            normalized_discrepancies
        ),
        "old_v1_tolerance_pass_count": old_v1_tolerance_pass_count,
        "old_v1_tolerance_failure_count": (
            EXPECTED_CANDIDATE_COUNT - old_v1_tolerance_pass_count
        ),
        "old_v1_tolerance_is_audit_only": True,
        "parameter_integrity": {
            "status": (
                "PASS_NO_PARAMETER_UPDATE"
                if parameter_integrity_pass
                else "FAIL"
            ),
            "trainable_sha256_before": preflight_trainable_sha_before,
            "trainable_sha256_after": preflight_trainable_sha_after,
            "frozen_sha256_before": frozen_parameter_sha_before,
            "frozen_sha256_after": preflight_frozen_sha_after,
        },
    }
    preflight_rows_path = output_dir / "stage6b_3_v2_preflight_candidates.jsonl"
    preflight_summary_path = output_dir / "stage6b_3_v2_preflight_summary.json"
    if atomic_jsonl(preflight_rows_path, preflight_rows) != EXPECTED_CANDIDATE_COUNT:
        raise RuntimeError("V2 preflight output row count is not exactly 100")
    atomic_json(preflight_summary_path, preflight_summary)

    print(json.dumps(preflight_summary, ensure_ascii=False, indent=2, sort_keys=True))
    if not preflight_pass:
        raise RuntimeError(
            "V2 no-update vector reproducibility preflight failed; "
            "no positive-eta update was performed"
        )
    print("v2_no_update_vector_preflight: PASS")
    ...
    results = []
    max_dot_error = 0.0
    max_cell_normalized_discrepancy = 0.0
    ...
    with reference_audit_temp.open("w", encoding="utf-8") as reference_handle:
        for candidate_position, candidate_index in enumerate(candidate_indices, start=1):
            ...
            norm_product_denominator = finite_number(
                frozen_candidate_norm * frozen_reference_norm,
                "frozen_gradient_norm_product",
            )
            if norm_product_denominator <= 0.0:
                raise RuntimeError(
                    f"Candidate {candidate_index}: frozen norm product is not positive"
                )
            ...
            for eta in eta_values:
                ...
                recomputed_dot = gradient_dot(candidate_gradient, reference_sum)
                dot_error = finite_number(
                    abs(recomputed_dot - frozen_dot),
                    "gradient_dot_absolute_error",
                )
                normalized_discrepancy = finite_number(
                    dot_error / norm_product_denominator,
                    "gradient_dot_normalized_discrepancy",
                )
                old_v1_dot_tolerance = max(1e-6, 1e-4 * abs(frozen_dot))
                max_dot_error = max(max_dot_error, dot_error)
                max_cell_normalized_discrepancy = max(
                    max_cell_normalized_discrepancy,
                    normalized_discrepancy,
                )
                if normalized_discrepancy > NORMALIZED_DOT_DISCREPANCY_MAX:
                    raise RuntimeError(
                        f"Candidate {candidate_index}, eta {eta}: gradient-dot "
                        f"normalised discrepancy {normalized_discrepancy} exceeds "
                        f"{NORMALIZED_DOT_DISCREPANCY_MAX}"
                    )
                ...
                predicted_change_discrepancy = abs(
                    recomputed_predicted_change - predicted_change
                )
                predicted_change_discrepancy_limit = (
                    eta
                    * NORMALIZED_DOT_DISCREPANCY_MAX
                ...
                result = {
                    ...
                    "frozen_gradient_norm_product": norm_product_denominator,
                    "normalized_dot_discrepancy": normalized_discrepancy,
                    "v2_normalized_discrepancy_limit": (
                        NORMALIZED_DOT_DISCREPANCY_MAX
                    ),
                    "old_v1_gradient_dot_tolerance": old_v1_dot_tolerance,
                    "within_old_v1_gradient_dot_tolerance": (
                        dot_error <= old_v1_dot_tolerance
                    ),
                    ...
                    "predicted_change_absolute_discrepancy": (
                        predicted_change_discrepancy
                    ),
                    "predicted_change_discrepancy_limit": (
                        predicted_change_discrepancy_limit
    ...
    run_config = {
        ...
        "protocol_amendment_commit": EXPECTED_AMENDMENT_COMMIT,
        "protocol_amendment_sha256": EXPECTED_SHA256["protocol_amendment"],
        ...
        "v2_reproducibility_spearman_minimum": (
            REPRODUCIBILITY_SPEARMAN_MIN
        ),
        "v2_normalized_dot_discrepancy_maximum": (
            NORMALIZED_DOT_DISCREPANCY_MAX
        ),
        "preflight_candidate_count": EXPECTED_CANDIDATE_COUNT,
        "preflight_positive_eta_update_performed": False,
        "preflight_scientific_endpoint_evaluated": False,
        "preflight_candidates_sha256": file_sha256(preflight_rows_path),
    ...
    summary = {
        ...
        "max_cell_normalized_dot_discrepancy": (
            max_cell_normalized_discrepancy
        ),
        "v2_reproducibility_preflight": preflight_summary,
        ...
        "validity_checks": {
            ...
            "v2_all_100_no_update_vector_preflight": "PASS",
            "v2_per_cell_normalized_dot_discrepancy": "PASS",
            "old_v1_gradient_dot_tolerance": "AUDIT_ONLY_NOT_PASS_FAIL",
            "frozen_stage6b_1_dot_retained_as_predictor": "PASS",
    ...
    print(f"v2_preflight_summary_path: {preflight_summary_path}")
    ...
    print(f"stage6b_3_v2_{args.mode}: PASS")
